...
class CustomDataset(Dataset):
    def __init__(self, root_dir='/home/datasets/customdata', classes=[],
        ...
        self.synth_id_to_category = {}
        self.category_to_synth_id = {}
        self.synth_id_to_number = {}

        for f in listdir(self.root_dir):
            self.synth_id_to_category[f]=f
            self.category_to_synth_id[f]=f

        for i, k in enumerate(self.synth_id_to_category.keys()):
            self.synth_id_to_number[k] = i
        ...
        if classes:
            ...
            classes = self.synth_id_to_category.keys()
        ...
        self.point_clouds_names_train = pd.concat([pc_df[pc_df['category'] == c][:int(1.0*len(pc_df[pc_df['category'] == c]))].reset_index(drop=True) for c in classes])
        self.point_clouds_names_valid = pd.concat([pc_df[pc_df['category'] == c][int(0.00*len(pc_df[pc_df['category'] == c])):int(1.0*len(pc_df[pc_df['category'] == c]))].reset_index(drop=True) for c in classes])
        self.point_clouds_names_test = pd.concat([pc_df[pc_df['category'] == c][int(0.0*len(pc_df[pc_df['category'] == c])):].reset_index(drop=True) for c in classes])
    ...
    def _get_names(self) -> pd.DataFrame:
        filenames = []
        for category_id in self.synth_id_to_category.keys():
